...
import {
  FiFileText, FiCheck, FiAlertTriangle, FiUpload, FiClock,
  FiAlertCircle, FiShield, FiPackage, FiTruck, FiChevronRight,
  FiCalendar, FiBox, FiChevronLeft,
} from "react-icons/fi";
import {
  Card, CardHeader, Input, BtnPrimary, BtnSecondary,
} from "../SalesComponent/ui/index";
import { db } from "../../firebase";
import {
  collection, getDocs, query, orderBy, addDoc, updateDoc, doc,
...
// ── Format date to "12 Mar 2026" style for dropdown label ────────
...
// ── Get YYYY-MM-DD from any date value ───────────────────────────
      ...
      >
        ...
        {selected !== "all" && (
          <span
            ...
            className="ml-1 w-4 h-4 rounded-full bg-white/25 hover:bg-white/50 flex items-center justify-center text-white text-xs leading-none"
            title="Clear"
          >×</span>
        )}
      </button>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-72 bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-indigo-600">
            <button onClick={prevMonth} className="w-7 h-7 rounded-full hover:bg-white/20 flex items-center justify-center text-white transition-colors">
              <FiChevronLeft size={15} />
            </button>
            <span className="text-sm font-bold text-white">
              {MONTHS_CAL[view.month]} {view.year}
            </span>
            <button onClick={nextMonth} className="w-7 h-7 rounded-full hover:bg-white/20 flex items-center justify-center text-white transition-colors">
            ...
            {cells.map((day, idx) => {
              ...
              return (
                <button
                  ...
                  disabled={!hasData}
                  className={`relative mx-auto w-8 h-8 rounded-full text-xs font-semibold flex items-center justify-center transition-all
                    ${isSel
                      ? "bg-indigo-600 text-white shadow-md"
                      : hasData
                        ? "text-slate-800 hover:bg-indigo-50 hover:text-indigo-700 cursor-pointer"
          ...
          <div className="border-t border-slate-100 px-4 py-2.5 flex items-center justify-between">
            <p className="text-[10px] text-slate-400">
              {activeDates.length} date{activeDates.length !== 1 ? "s" : ""} have POs
            </p>
            {selected !== "all" && (
...
function POHistoryTimeline({ selectedPO, linkedInvoices, loadingHistory }) {
  ...
  const events = [];
  events.push({
    type: "created", icon: "📄", label: "Purchase Order Created",
  ...
  [...linkedInvoices]
    ...
    .forEach((inv) => {
      ...
      events.push({
        ...
        status: inv.poStatus || "partial", pending, damagedItems,
      });

      if (inv.poStatus === "partial")
        events.push({ type: "status", icon: "🔄", label: "Status changed → PARTIAL", sub: `${pending} units still pending`, datetime: inv.createdAt, status: "partial", pending });
      else if (inv.poStatus === "complete")
        events.push({ type: "status", icon: "✅", label: "Status changed → COMPLETE", sub: `All ${invOrdered} units received`, datetime: inv.createdAt, status: "complete" });
      else if (inv.poStatus === "excess")
        events.push({ type: "status", icon: "⚠️", label: "Status changed → EXCESS", sub: `Received more than ordered`, datetime: inv.createdAt, status: "excess" });
    });
                ...
                <div className="min-w-0">
                  ...
                  {event.type === "invoice" && (
                    <div className="flex items-center gap-3 mt-1 flex-wrap">
                      ...
                      {event.qc && (
                        <span className={`text-[10px] font-bold ${event.qc === "passed" ? "text-emerald-600" : event.qc === "failed" ? "text-red-600" : "text-orange-600"}`}>
          ...
          <div className="w-full max-w-xs space-y-3 text-left">
            {[
              ...
              <div key={i} className="flex items-center gap-3">
                ...
                <span className={`text-xs ${s.done ? "text-emerald-700 font-medium" : s.active ? "text-indigo-700 font-bold" : "text-slate-400"}`}>
                  {s.label}
                </span>
              </div>
            ))}
...
export default function UploadVendorInvoice() {
  ...
  // ✅ Unique dates from PO list (sorted desc)
  const availableDates = [...new Set(
    pendingPOs
      .map(po => getDateKey(po.createdAt))
      .filter(Boolean)
  )].sort((a, b) => b.localeCompare(a));

  // ✅ Filtered POs based on selected date
  const filteredPOs = selectedDate === "all"
    ? pendingPOs
        ...
        if (selectedPO) {
          setReceivedItems(
            selectedPO.items.map((poItem) => {
              ...
              return {
                ...poItem, orderedQty, alreadyReceived: already,
    ...
    try {
      ...
      if (savedInvoiceId) {
        await updateDoc(doc(db, "excelupload", savedInvoiceId), {
          qualityCheck, remarks, salesConfirmedAt: now, poStatus: displayPoStatus,
        });
      }
      setUploading(false);
      setStep(5);
  ...
  const getTotalShortage = () =>
    receivedItems.reduce((sum, item) => sum + Math.max(0, (item.orderedQty || 0) - ((item.alreadyReceived || 0) + (item.newReceived || 0))), 0);
  const getTotalNewReceived = () =>
    receivedItems.reduce((sum, item) => sum + (item.newReceived || 0), 0);

  const livePoStatus = (() => {
    const c = calcPoStatus(receivedItems.map((i) => ({
          ...
          {loadingPOs ? (
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-14 text-center">
              ...
              <p className="text-sm font-bold text-slate-600">
                {selectedDate !== "all" ? `${formatDateLabel(selectedDate)} na koi PO nathi` : "No Pending Purchase Orders"}
              </p>
              <p className="text-xs text-slate-400 mt-1">
                {selectedDate !== "all"
                  ? <button onClick={() => setSelectedDate("all")} className="text-indigo-500 hover:underline font-semibold">All dates jouo</button>
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="flex items-start gap-3 p-3 bg-indigo-50 border border-indigo-200 rounded-lg">
              ...
              <p className="text-xs text-indigo-700">
                Clicking <strong>"Submit for Store QC"</strong> will send this invoice to the Store team for quality verification. Stock will update only after their approval.
              </p>
            </div>
          </div>
        </Card>
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-50 border border-amber-200">
                ...
                <p className="text-xs text-amber-800">
                  Once submitted, this invoice will be <strong>locked for editing</strong>. Stock records are already updated by Store.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="p-12 text-center">
            ...
            <div className="flex items-center justify-center gap-3 flex-wrap">
              ...
              <BtnSecondary onClick={() => {
                setStep(1); setSelectedPO(null); setReceivedItems([]); setExcelParsed(false);
                setInvoiceExcelFile(null); setInvoiceNo(""); setInvoiceHeader(null);
                setLinkedInvoices([]); setDuplicateWarning(""); setSavedInvoiceId(null);
              }}>
