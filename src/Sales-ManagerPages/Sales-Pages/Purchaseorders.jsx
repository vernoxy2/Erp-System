...
function StatusPill({ status }) {
  const map = {
    ...
    waiting_qc: "bg-indigo-50 text-indigo-700 border-indigo-200",  // ✅ NEW
  };
  const s = (status || "ordered").toLowerCase();
  // ✅ waiting_qc label readable format
  const label = s === "waiting_qc" ? "Waiting QC" : s.replace("_"," ");
  return (
    <span className={`px-2.5 py-0.5 text-[10px] font-bold rounded-full border uppercase ${map[s] || map.ordered}`}>
      {label}
    </span>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[88vh] flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between flex-shrink-0">
          <div>
            <div className="flex items-center gap-3">
              <p className="text-sm font-black text-slate-800">PO Items — {po.poNumber}</p>
              <StatusPill status={po.status}/>
            </div>
            <p className="text-xs text-slate-400 mt-0.5">
              {po.vendor} · {po.items?.length||0} items · Date: {po.date}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-50">
              {(po.items||[]).map((item, idx) => {
                ...
                return (
                  <tr key={idx} className="hover:bg-slate-50/60 transition-colors">
                    <td className="px-4 py-3 text-xs text-slate-400 font-semibold">{item.slNo || idx+1}</td>
                    <td className="px-4 py-3">
                      <span className="text-xs font-bold font-mono text-slate-700 bg-slate-100 px-2 py-0.5 rounded-full">
                        {item.productCode || "—"}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-xs text-slate-600 max-w-[200px]">{item.description || "—"}</td>
                    <td className="px-4 py-3 text-center text-xs text-slate-400 font-mono">{item.hsnSac || "—"}</td>
                    <td className="px-4 py-3 text-center font-black text-slate-800 text-sm">{ordered}</td>
                    <td className="px-4 py-3 text-center font-bold text-blue-600">
                      {received > 0 ? received : <span className="text-slate-300 font-normal">—</span>}
                    </td>
                    <td className="px-4 py-3 text-center">
                    ...
                    <td className="px-4 py-3 text-center text-xs text-slate-400 uppercase">{item.unit || "nos"}</td>
                    <td className="px-4 py-3 text-center"><StatusPill status={istatus}/></td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <div className="px-6 py-3 border-t border-slate-100 bg-slate-50/60 flex-shrink-0">
          <div className="flex items-center gap-6 text-xs">
            <span className="text-slate-500">Ordered: <strong className="text-slate-800">{totalOrdered}</strong></span>
            <span className="text-slate-500">Received: <strong className="text-blue-600">{totalReceived}</strong></span>
            <span className="text-slate-500">Pending: <strong className={totalPending > 0 ? "text-orange-500" : "text-emerald-600"}>
              {totalPending > 0 ? totalPending : "All received ✓"}
            </strong></span>
          </div>
        </div>
...
export default function PurchaseOrders() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      ...
      (snapshot) => {
        ...
        const mapped = pos.map((po) => {
          const { status: etaStatus, remainingDays } = calcEtaStatus(po.deliveryDate);
          // ✅ storeQcPending takes priority — show Waiting QC regardless of poStatus
          const poStatus = po.storeQcPending
            ? "waiting_qc"
          ...
          return {
            ...
            createdAt:    po.createdAt || null,
            excelHeader:  po.excelHeader || {},
            storeQcPending: po.storeQcPending || false,  // ✅ NEW
            items: (po.items||[]).map((item) => ({
              ...item,
              orderedQty:       item.orderedQty || item.quantity || 0,
              totalReceivedQty: item.totalReceivedQty || 0,
  ...
  const partialCount   = purchaseOrders.filter((p) => p.status==="partial").length;
  const overdueCount   = purchaseOrders.filter((p) => p.status==="overdue").length;
  const waitingQcCount = purchaseOrders.filter((p) => p.status==="waiting_qc").length; // ✅ NEW
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ✅ NEW: Waiting QC Banner */}
      {!loading && waitingQcCount > 0 && (
        <div className="flex items-center gap-3 px-5 py-3 bg-indigo-50 border border-indigo-200 rounded-xl">
          <span className="text-lg animate-pulse">⏳</span>
          <p className="text-sm font-bold text-indigo-800">
            {waitingQcCount} PO{waitingQcCount > 1 ? "s" : ""} waiting for Store QC approval
          </p>
          <p className="text-xs text-indigo-500 ml-auto">Stock will update once approved</p>
        </div>
      )}
        ...
        <div className="px-6 py-4 flex items-center justify-between gap-4 border-b border-slate-100">
          ...
          <div className="flex items-center gap-3">
            ...
            <select
              ...
              className="px-3 py-2 text-xs border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-white"
            >
              {/* ✅ waiting_qc option added */}
              {["All Status","ordered","partial","complete","received","excess","overdue","warning","waiting_qc"].map((s) => (
                <option key={s} value={s}>
                  {s==="All Status" ? "All Status" : s==="waiting_qc" ? "Waiting QC" : s.charAt(0).toUpperCase()+s.slice(1)}
                </option>
              ))}
            </select>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {filtered.map((po) => {
                  const totalOrdered  = po.items.reduce((s,i)=>s+(i.orderedQty||0),0);
                  const totalReceived = po.items.reduce((s,i)=>s+(i.totalReceivedQty||0),0);
                  const isWaiting     = po.status === "waiting_qc"; // ✅ NEW
                  const rowBg =
                    isWaiting         ? "bg-indigo-50/30"  :  // ✅ NEW
                    po.status==="overdue" ? "bg-red-50/40"    :
                    po.status==="warning" ? "bg-orange-50/30" :
                    po.status==="partial" ? "bg-orange-50/20" : "";
                  ...
                  return (
                    <tr key={po.id} className={`hover:bg-slate-50/60 transition-colors ${rowBg}`}>
                      ...
                      <td className="px-4 py-4 text-center">
                        <StatusPill status={po.status}/>
                        {/* ✅ NEW: waiting_qc sub-label */}
                        {isWaiting && (
                          <p className="text-[10px] text-indigo-500 font-bold mt-1 animate-pulse">Store reviewing...</p>
                        )}
                        {po.status==="partial" && totalOrdered-totalReceived > 0 && (
                          <p className="text-[10px] text-orange-500 font-bold mt-1">
                            {totalOrdered-totalReceived} pending
                      ...
                      {/* View → Items Table Modal (unchanged) */}
                      <td className="px-4 py-4 text-center">
                        <button
                          onClick={() => setViewPO(po)}
                          className="flex items-center gap-1.5 mx-auto text-teal-600 hover:text-teal-800 text-xs font-semibold bg-teal-50 hover:bg-teal-100 px-2.5 py-1.5 rounded-lg transition-colors"
                      ...
                      {/* ✅ Action: waiting → "View Status", else → "Edit" */}
                      <td className="px-4 py-4 text-center">
                        {isWaiting ? (
                          <button
                            onClick={() => navigate(`/sales/purchase-orders/upload-invoice?poId=${po.id}`)}
                            className="flex items-center gap-1.5 mx-auto text-indigo-600 hover:text-indigo-800 text-xs font-semibold bg-indigo-50 hover:bg-indigo-100 px-2.5 py-1.5 rounded-lg transition-colors"
                          >
                            <span className="animate-pulse">⏳</span> View Status
                          </button>
                        ) : (
                          <button
                            onClick={() => navigate(`/sales/purchase-orders/upload?poId=${po.id}&editMode=true`)}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { color:"bg-orange-500",  label:"Partial",    desc:"Some material received" },
            { color:"bg-emerald-500", label:"Complete",   desc:"All material received" },
            { color:"bg-teal-500",    label:"Received",   desc:"Material received & confirmed" },
            { color:"bg-purple-500",  label:"Excess",     desc:"Extra material received" },
            { color:"bg-orange-400",  label:"Warning",    desc:"2 days or less" },
            { color:"bg-red-500",     label:"Overdue",    desc:"ETA passed" },
            { color:"bg-indigo-500",  label:"Waiting QC", desc:"Awaiting store QC approval" }, // ✅ NEW
          ].map((item) => (
            <div key={item.label} className="flex items-center gap-2.5">
              <div className={`w-3 h-3 rounded-full ${item.color} flex-shrink-0`}/>
              <div>
