      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[88vh] flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-3 border-t border-slate-100 bg-slate-50/60 flex-shrink-0">
          <div className="flex items-center gap-6 text-xs">
            <span className="text-slate-500">
              Ordered:{" "}
              <strong className="text-slate-800">{totalOrdered}</strong>
            </span>
            <span className="text-slate-500">
              Received:{" "}
              <strong className="text-blue-600">{totalReceived}</strong>
            </span>
            <span className="text-slate-500">
              Pending:{" "}
              <strong
                className={
                  totalPending > 0 ? "text-orange-500" : "text-emerald-600"
                }
              >
                {totalPending > 0 ? totalPending : "All received ✓"}
              </strong>
...
export default function PurchaseOrders() {
      ...
      (snapshot) => {
        ...
        const mapped = pos.map((po) => {
          ...
          return {
            ...
            remainingDays,
            // createdAt: po.createdAt || null,
            createdAt: po.createdAt?.toDate?.() ?? po.createdAt ?? null,
            excelHeader: po.excelHeader || {},
  ...
  const waitingQcCount = purchaseOrders.filter(
    (p) => p.status === "waiting_qc",
  ).length; // ✅ NEW
      ...
      {!loading && (
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            <div
              key={c.label}
              className={`bg-white rounded-2xl border ${c.border} p-5 flex items-center gap-4 shadow-sm`}
            >
              <div
                className={`${c.bg} text-white p-3 rounded-xl flex-shrink-0`}
              >
                {c.icon}
              </div>
              <div>
                ...
                <p className="text-3xl font-black mt-0.5 text-slate-800">
                  {c.value}
                </p>
              </div>
            </div>
      ...
      {!loading && waitingQcCount > 0 && (
        <div className="flex items-center gap-3 px-5 py-3 bg-indigo-50 border border-indigo-200 rounded-xl">
          ...
          <p className="text-sm font-bold text-indigo-800">
            {waitingQcCount} PO{waitingQcCount > 1 ? "s" : ""} waiting for Store
            QC approval
          </p>
          <p className="text-xs text-indigo-500 ml-auto">
            Stock will update once approved
          </p>
        </div>
      )}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-4 text-center">
                        ...
                        {isWaiting && (
                          <p className="text-[10px] text-indigo-500 font-bold mt-1 animate-pulse">
                            Store reviewing...
                          </p>
                        )}
                        {po.status === "partial" &&
                      ...
                      <td className="px-4 py-4 text-center">
                        {isWaiting ? (
                          <button
                            onClick={() =>
                              navigate(
                                `/sales/purchase-orders/upload-invoice?poId=${po.id}`,
                              )
                            }
                            className="flex items-center gap-1.5 mx-auto text-indigo-600 hover:text-indigo-800 text-xs font-semibold bg-indigo-50 hover:bg-indigo-100 px-2.5 py-1.5 rounded-lg transition-colors"
                          >
                            <span className="animate-pulse">⏳</span> View
                            Status
                          </button>
                        ) : (
                          <button
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            },
            {
              color: "bg-teal-500",
              label: "Received",
              desc: "Material received & confirmed",
            },
            {
              color: "bg-purple-500",
              label: "Excess",
            ...
            { color: "bg-red-500", label: "Overdue", desc: "ETA passed" },
            {
              color: "bg-indigo-500",
              label: "Waiting QC",
              desc: "Awaiting store QC approval",
            }, // ✅ NEW
          ].map((item) => (
            <div key={item.label} className="flex items-center gap-2.5">
              <div
